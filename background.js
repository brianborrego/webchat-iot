// FaultyTerminal background - vanilla JS port (uses OGL)
(function () {
  const vertexShader = `
  ...
  const renderer = new ogl.Renderer({ dpr: Math.min(window.devicePixelRatio || 1, 2) });
  ...
  const program = new ogl.Program(gl, {
    ...
    uniforms: {
      iTime: { value: 0 },
      iResolution: { value: new ogl.Color(gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height) },
      uScale: { value: 1.5 },
      ...
      uTint: { value: new ogl.Color(0.655, 0.937, 0.620) }, // #A7EF9E
  ...
  function resize() {
    ...
    program.uniforms.iResolution.value = new ogl.Color(
  ...
  function update(t) {
    ...
    const elapsed = (t * 0.001 + timeOffset) * 0.5;
    program.uniforms.iTime.value = elapsed;

    const animDuration = 2000;
    const animElapsed = t - loadStart;
    program.uniforms.uPageLoadProgress.value = Math.min(animElapsed / animDuration, 1);
